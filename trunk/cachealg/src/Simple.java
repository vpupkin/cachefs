...
public class Simple 
{
	...
	public Simple(int r, int s, Vector<FileType> f, Vector<Integer> p, Times t)
	{
		ram = r;
		ssd = s;
		files = f;
	...
	void insert() throws IOException
	{
		...
		ssdtmp = ssd;
		n = profile.size();
		ps = new PrintStream( new FileOutputStream("Results1.txt"));
		...
		for (i=0;i<n;i++)
		{
			...
			{
				timetmp = times.ram_acc + size/times.ram_rate;	//partial time
				ps.print(index+" ");
				ps.print(size+" ");
				ps.println(timetmp+" ");
				time += timetmp;								//total time
			}
			...
			else											//the file will be copied in ram or ssd (if it fits)
			{
				...
				else if (size < ssdtmp) 					//we have space in ssd
				{ 
					ssdtmp-=size;
				...
				else										//we don't have neither in ram or in ssd		
				{
					if (size<ram)							//the file can fit in ram
					{
					...
					else									//the file can't fit in ram or ssd and is accessed from hdd
					{
