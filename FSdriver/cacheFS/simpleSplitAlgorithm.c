...
#define SIZE_THRESHOLD	1*1024*1024		/* Threshold separating the small and large files categories */


void recordAccess_simpleSplitAlg(const char*  path, const char* hddPath );
int getAction_simpleSplitAlg(const char* path );


int initCacheStructs_simpleSplitAlg(void* parameters)
{
	struct params_simpleSplitAlg* p= parameters;

	printf("initCacheStructs_simpleSplitAlg \n");
...
int releaseCacheStructs_simpleSplitAlg()
{
	free(fsRoot);
	free(ssdMountRoot);
	free(ramMountRoot);
...
int initCachingAlgorithm_simpleSplitAlg(cachingAlgoritm* alg, cacheControl* ccontrol)
{
	...
	alg->initCacheStructs = initCacheStructs_simpleSplitAlg;
	alg->releaseCacheStructs = releaseCacheStructs_simpleSplitAlg;

	alg->recordAccess = recordAccess_simpleSplitAlg;
	alg->getAction = getAction_simpleSplitAlg;
...
int getAction_simpleSplitAlg(const char* path )
{
	...
	if (r->isSSD)
		return READ_FROM_SSD;				// file is in SSD, read directly, no further analysis needed


	// file is not cached, analyze where to place it
